...
function generateNewWords() {
    const allWords = ['JAVA', 'HTML', 'CSS', 'JAVASCRIPT', 'PYTHON', 'REACT', 'NODE', 'EXPRESS', 'ANGULAR', 'PHP', 'C++', 'SQL', 'RUBY', 'SWIFT', 'GO', 'DESIGN', 'ANALYSIS', 'ALGORITHM'];
    const randomWords = [];
        ...
        for (let row = 0; row < gridSize; row++) {
            for (let col = 0; col < gridSize; col++) {
                for (let direction = 0; direction < 2; direction++) { // 0: horizontal, 1: vertical
                    if (canPlaceWord(grid, word, row, col, direction)) {
                        placeWord(grid, word, row, col, direction); // Try placing the word
                        if (placeWordsRecursively(index + 1)) return true; // Recur for the next word
                        removeWord(grid, word, row, col, direction); // Backtrack
                    }
                }
            }
        }

        return false; // Backtrack if no placement is possible
    }
