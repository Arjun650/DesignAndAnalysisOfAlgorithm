class BranchAndBoundSolver {
  ...
  traversable(row, col) {
    return (
      0 <= row && row < this.#rowNumber && 0 <= col && col < this.#colNumber
    );
  }
    ...
    for (let row = 0; row < this.#rowNumber; row += 1) {
      for (let col = 0; col < this.#colNumber; col += 1) {
        if (this.#grid[row][col] === this.#target[0]) {
          ...
          this.#marker.animate();
          this.#state.set(
            `Adding (${row},${col}) to the queue as a potential start point`
          );
    ...
    while (queue.length > 0) {
      ...
      for (const [deltaRow, deltaCol] of this.#directions) {
        ...
        if (
          ...
          this.#marker.animate();
          this.#state.set(
            `(${newRow},${newCol}) does not match or is not traversable`
          );
        }
      }
    }
...
//             this.#marker.queueMark({ row, col });
//             this.#marker.animate();
//             this.#state.set(`start branch at (${row},${col}) with (${deltaRow},${deltaCol}) direction`);

//             if (this.search(row, col, 0, deltaRow, deltaCol)) {
//               this.#state.set("correct words. mark solution!");
//             } else {
//               this.#state.set("pruned branch. move to next!");
//             }
//           }
//         }
//       }
...
//   search(row, col, index, deltaRow, deltaCol) {
//     if (index === this.#targetLength) return true; // Target word found
//     if (!this.traversable(row, col) || this.#grid[row][col] !== this.#target[index]) return false; // Prune branch

//     this.#marker.queueMark({ row, col });
//     this.#marker.animate();

//     const result = this.search(row + deltaRow, col + deltaCol, index + 1, deltaRow, deltaCol);

//     if (result) {
//       this.#answer.push({ row, col, dir: [deltaRow, deltaCol] });
//       this.#marker.queueMark({ row, col, answer: true });
//     } else {
